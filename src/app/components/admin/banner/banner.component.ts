  ...
  onDelete(bannerId: number):void{
    Swal.fire({
      ...
      if(result.value){
          ...
          this.bannerService.deleteImgBanner(bannerId).pipe(takeUntil(this.destroy$)).subscribe(res=> {
            Swal.fire('Eliminado!!',' El banner ahora es inactivo.','success');
            this.bannerService.filteBannerr('genial');
          });
      }
   ...
   return (data: any, filter: string) => {
      ...
      if(!DateStarted && !DateEnd){
         ...
         this.puchArrayToPrint(filterData, data,matchFilter);
          ...
          filterData= (data.ci_nit.trim().toLowerCase() == buscar) && ((fechapersonalizado >= DateStarted) && (fechapersonalizado <= DateEnd));
