...
type TableRow = [ number, string, string, string, string, string, string, string, string];
...
export class UsersComponent implements AfterViewInit, OnInit, OnDestroy, OnChanges {
  ...
  onDelete(userId: number):void{
    ...
    Swal.fire({
      ...
      if(result.value){
          ...
          this.userService.deleteUser(userId).pipe(takeUntil(this.destroy$)).subscribe(res=> {
  ...
  getFilterPredicate(){
   ...
   return (data: any, filter: string) => {
      ...
      if(!DateStarted && !DateEnd){
         ...
         filterData=  data.logins[0].fecha >= DateStarted && data.logins[0].fecha <= DateEnd;
         ...
         this.puchArrayToPrint(filterData, data,matchFilter);
          ...
          filterData= (data.rol.nombre.trim().toLowerCase() == buscar) && ((data.logins[0].fecha >= DateStarted) && (data.logins[0].fecha <= DateEnd));
  ...
  //----------------------------------create pdf-----------
   ...
   createTable(datos: Users[]): ITable{
     [{}]
     return new Table([
       ['CÉDULA IDENTIDAD','EXPEDIDO','NOMBRE',' APELLIDO PATERNO','APELLIDO MATERNO','CELULAR','DIRECCIÓN','CIUDAD','ESTADO'],
   ...
   extractData(datos: Users[]): TableRow[]{
     ...
     return datos.map(row =>[row.ci, row.expedido, row.nombre, row.paterno, row.materno,  row.celular, row.direccion,row.ciudad, row.estado]);
