...
export class ProductoComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.hoymas30dias = moment(new Date(new Date()))
      .add(30, 'days')
      .format('YYYY-MM-DD');
  }
  ...
  async createPdf() {
    ...
    pdf.add(
      await new Img('../../../../assets/logoagrandado.png')
        .alignment('center')
        .width(130)
        .height(130)
        .build()
    ...
    pdf.add(
      new Txt(
        'Todos los productos - por vencer - categoria - fecha vencimiento'
      )
        .alignment('center')
        .fontSize(12).end
    );
