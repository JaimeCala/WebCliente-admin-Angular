...
type TableRow = [ number, number, number ,number, string];
...
export class ModalpedidoComponent implements OnInit, AfterViewInit, OnDestroy {
 ...
 actionTODO= Action.NEW;
  ...
  ngOnInit(): void {
     ...
     //--------inicializa vendedor----//;
    this.vendedorService.getTodosVendedorUsers().subscribe((vendedor)=>{ this.myVendedor=vendedor});
 ...
 checkFieldRepartidor(field: string): boolean{
  ...
  //------------------------generar pdf-----------------//
   ...
   createTable(datos: PedidoProductoPdf[]): ITable{
     [{}]
     return new Table([
       ['ID','CANTIDAD','PRECIO UNIDAD','PRECIO TOTAL','NOMBRE PRODUCTO'],
   ...
   extractData(datos: PedidoProductoPdf[]): TableRow[]{
     ...
     return datos.map(row =>[row.idpedidoproducto,row.cantidad, row.precio_uni,row.precio_total, row.productonombre]);
